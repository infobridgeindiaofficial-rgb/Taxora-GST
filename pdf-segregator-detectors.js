// PDF Segregator — marketplace + date detection rules.
// Kept isolated from the UI so the rules can be tuned/extended later
// (e.g. with real sample shipping-label PDFs) without touching pdf-segregator.html.
//
// NOTE: These rules were written from publicly known label conventions
// (Amazon/Flipkart/Meesho order-id formats, common label header text).
// They have NOT been verified against real Amazon/Flipkart/Meesho sample
// PDFs (none were available in this repo at implementation time). Treat
// the confidence threshold as conservative-by-design: ambiguous or weak
// matches fall back to "Needs Review" rather than guessing.
...
const MARKETPLACE_RULES = {
  ...
  Meesho: [
    // Real Meesho "Sub_Order_Labels_*.pdf" invoices (13 verified samples) never print
    // the word "Meesho" anywhere — the seller generates the bill of supply, not the
    // marketplace. These structural signals were confirmed present in all 13 real
    // samples and absent from every real Amazon/Flipkart sample checked.
    { pattern: /\b\d{15,19}_\d{1,3}\b/, weight: 2 }, // Meesho sub-order id, e.g. 332318070202076544_1
...
// Explicit priority order of "meaningful" date labels, highest priority first.
// "Order Date" is ranked first based on real Amazon invoice/label samples: it is
// the one field that is reliably present and correctly reflects the useful
// order/shipment date across every sample seen (see DATE_PATTERNS note below for
// the specific format evidence that drove the "." separator support).
...
// How far past a label's end a date value is allowed to be to count as "belonging"
// to that label, e.g. "Order Date: 12.08.2026" — label ends right before the value.
...
const DATE_PATTERNS = [
  ...
  {
    // 22/09/2026, 22-09-2026 or 22.08.2026 (day-month-year, the common Indian
    // convention). The "." separator was added after inspecting real Amazon
    // invoices, which print e.g. "Order Date: 12.08.2026" — the earlier version
    // of this regex only accepted "/" and "-" and silently missed every dotted date.
    regex: /\b(\d{1,2})[.\/\-](\d{1,2})[.\/\-](\d{4})\b/g,
    resolve: (m) => {
      const a = Number(m[1]), b = Number(m[2]), year = Number(m[3]);
      // Prefer day-month-year; if the first part can't be a day (>31) or
      // the second can't be a month (>12), don't guess a swapped reading.
...
// Walks DATE_LABEL_PRIORITY in order and, for the first label that has a date
// value shortly after it in the text, returns that date. This is an explicit
// priority order based on label meaning — not "whichever date appears first".
...
if (typeof window !== 'undefined') {
  window.TaxoraPdfDetectors = TaxoraPdfDetectors;
}
